...
describe('Companion config', () => {
	...
	it('clamps the poll fallback interval to the supported range', () => {
		assert.equal(resolvePollIntervalMs({ ...baseConfig, pollIntervalMs: Number.NaN }), 5000)
		assert.equal(resolvePollIntervalMs({ ...baseConfig, pollIntervalMs: 100 }), 1000)
		assert.equal(resolvePollIntervalMs({ ...baseConfig, pollIntervalMs: 7000 }), 7000)
		assert.equal(resolvePollIntervalMs({ ...baseConfig, pollIntervalMs: 999999 }), 60000)
	})

	it('describes every config field', () => {
		for (const field of GetConfigFields()) {
			assert.ok('tooltip' in field && typeof field.tooltip === 'string' && field.tooltip.length > 0, field.id)
		}
	})
})
